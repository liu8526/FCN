...
class FCN(nn.Module):
    ...
    def forward(self, x):
        s1 = self.stage1(x)     #176*240*64 1/2
        s2 = self.stage2(s1)    #88*120*128 1/4
        s3 = self.stage3(s2)    #44*60*156  1/8
        s4 = self.stage4(s3)    #22*30*512  1/16
        s5 = self.stage5(s4)    #11*15*512  1/32

        scores1 = self.scores1(s5)  #11*15*num_classes
        ...
        add2 = add1 + s3
        output = self.conv_trans2(add2) #44*60*256
